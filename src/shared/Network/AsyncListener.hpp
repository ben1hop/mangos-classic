...
namespace MaNGOS
{
    ...
    class AsyncListener
    {
        public:
            ...
            AsyncListener(boost::asio::io_context& io_service, std::string const& bindIp, unsigned short port) : m_service(io_service), m_acceptor(io_service, boost::asio::ip::tcp::endpoint(boost::asio::ip::address::from_string(bindIp), port))
            {
                startAccept();
            }
